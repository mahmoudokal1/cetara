...
async function afterLogin(){
  ...
  if(!b.brand_name || !currentBrand.health_score){
    ...
    if(!b.onboarding_seeded){
      await seedEntriesFromOnboarding(); // recover users who finished onboarding before this fix existed
    }
    launchDashboard();
  }
}
...
function sf(k,v){
  ...
  saveBrandData();
  if(k==='brand_name'||k==='industry'||k==='model'){
...
function saveBrandData(){
  ...
  saveTimeout=setTimeout(flushBrandSave, 500);
}
async function flushBrandSave(){
  clearTimeout(saveTimeout);
  if(!pendingSave) return;
  pendingSave=false;
  const { error } = await sb.from('brands').update({ data: currentBrand.data }).eq('id', currentBrand.id);
...
// ══════════════════════════════════════
// ══════════════════════════════════════
// SEED ENTRIES FROM ONBOARDING DATA
// Converts the baseline numbers typed during onboarding into real
// entries rows, so the Dashboard / P&L / AI Advisor / charts all
// read from one single source of truth (the entries table) instead
// of two disconnected places.
// ══════════════════════════════════════
async function seedEntriesFromOnboarding(){
  const b = currentBrand.data || {};
  if(b.onboarding_seeded) return; // never duplicate on re-login

  const today = new Date().toISOString().split('T')[0];
  const rows = [];

  if(parseFloat(b.rev_now)>0) rows.push({brand_id:currentBrand.id,type:'revenue',date:today,description:'Revenue — this month (from onboarding)',category:'Shopify Orders',amount:parseFloat(b.rev_now),notes:'Auto-seeded from onboarding'});
  if(parseFloat(b.cogs_month)>0) rows.push({brand_id:currentBrand.id,type:'expense',date:today,description:'COGS — this month (from onboarding)',category:'Production',amount:parseFloat(b.cogs_month),notes:'Auto-seeded from onboarding'});
  if(parseFloat(b.rent)>0) rows.push({brand_id:currentBrand.id,type:'expense',date:today,description:'Rent / warehouse',category:'Misc',amount:parseFloat(b.rent),notes:'Auto-seeded from onboarding'});
  if(parseFloat(b.salaries)>0) rows.push({brand_id:currentBrand.id,type:'expense',date:today,description:'Salaries / freelancers',category:'Misc',amount:parseFloat(b.salaries),notes:'Auto-seeded from onboarding'});
  if(parseFloat(b.software)>0) rows.push({brand_id:currentBrand.id,type:'expense',date:today,description:'Tools & software',category:'Misc',amount:parseFloat(b.software),notes:'Auto-seeded from onboarding'});
  if(parseFloat(b.ads_spend)>0) rows.push({brand_id:currentBrand.id,type:'expense',date:today,description:'Meta Ads spend',category:'Marketing & Content',amount:parseFloat(b.ads_spend),notes:'Auto-seeded from onboarding'});
  if(parseFloat(b.ads_rev)>0) rows.push({brand_id:currentBrand.id,type:'revenue',date:today,description:'Revenue from ads',category:'Shopify Orders',amount:parseFloat(b.ads_rev),notes:'Auto-seeded from onboarding'});

  if(rows.length){
    const { data, error } = await sb.from('entries').insert(rows).select();
    if(!error && data){ allEntries = [...data, ...allEntries]; }
  }
  sf('onboarding_seeded', true);
  await flushBrandSave();
}

async function generateScore(){
  await flushBrandSave(); // ensure all typed fields are persisted before scoring
  const b = currentBrand.data || {};
  ...
  setTimeout(async ()=>{
    ...
    const { error: scoreSaveErr } = await sb.from('brands').update({ data: currentBrand.data, health_score: hs, brand_state: st[0] }).eq('id', currentBrand.id);
    if(scoreSaveErr){
      alert('Could not save your data: '+scoreSaveErr.message+'\n\nPlease try again before leaving this page.');
      btn.textContent='⚡ Generate Health Score'; btn.disabled=false;
      return;
    }
    ...
    btn.disabled=false;
    btn.onclick=async ()=>{
      btn.textContent='Setting up your dashboard…'; btn.disabled=true;
      await seedEntriesFromOnboarding();
      launchDashboard();
    };
